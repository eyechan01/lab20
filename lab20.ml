...
let threshold img threshold =
  List.map  (fun row ->
      List.map (fun v ->
          if v <= threshold then 0. else 1.) row) img

(* show the image *)
let depict img =
  Graphics.open_graph "";
  Graphics.clear_graph ();
  let x, y = List.length (List.hd img), List.length img
  in Graphics.resize_window x y;
  List.iteri (fun r row -> List.iteri (fun c pix ->
    let lvl = int_of_float (255. *. (1. -. pix))
...
let dither img =
  List.map (fun row ->
     List.map
       (fun v ->
          if v > Random.float 1. then 1. else 0.) row) img
